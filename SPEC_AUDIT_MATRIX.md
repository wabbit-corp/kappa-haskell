...
| 3.1.5A | provenance frames for generated syntax/obligations/implicit insertions/transports MUST | IMPLEMENTED-DIAGNOSTIC-FACING | TYP | `desugared-from` related origin on desugaring diagnostics; full KCore ProvenanceFrame store §34 profile-scoped (KNOWN_SPEC_ISSUES §13). (G10) |
...
| 28.2 | proof helpers `absurd pathInd subst sym trans cong unsafeAssertProof witness measureRelation lexRelation` | IMPLEMENTED (10/10) | RUN,V | all resolve with spec sigs (endpoint inference tested for sym/trans/cong; equality-match bodies still primitive, KNOWN_SPEC_ISSUES §17). Re-verified 2026-06-20: `measureRelation`/`lexRelation` now resolve as names (E_UNSOLVED_IMPLICIT on the `WellFoundedRelation` arg, not E_NAME_UNRESOLVED) — same shape as the other erased proof helpers. (G20 resolved) |
...
| G10 | 3.1.5A + 30.2.3 | RESOLVED-DIAGNOSTIC-FACING (was MAJOR) | desugarings blame user source as primary + `desugared-from` related origin (the `!e` splice); full KCore-node ProvenanceFrame store is §34 profile-scoped (documented) | DONE: diagnostic-facing frame in `Check.hs` `EBang`; boundary in `KNOWN_SPEC_ISSUES.md` §13 |
...
| G20 | 28.2 | RESOLVED (was MAJOR) | 10/10 resolve: `absurd subst sym trans cong pathInd unsafeAssertProof witness measureRelation lexRelation` all declared with spec signatures (endpoint inference tested for sym/trans/cong; equality-match bodies still primitive per KNOWN_SPEC_ISSUES §17). Re-verified 2026-06-20: `measureRelation`/`lexRelation` resolve as names (E_UNSOLVED_IMPLICIT on the `WellFoundedRelation` implicit, not E_NAME_UNRESOLVED) — the prior "still unresolved" residual was stale. | DONE in `src/Kappa/Prelude.hs`. |
| G21 | 28.2.3 | RESOLVED (instances present; Nat partiality is by-design) | re-verified 2026-06-20: `Double`/`Float` subtraction works (`subDefined=True`); `Nat` subtraction has a `CheckedSub Nat` instance (`subDefined = \x y -> leInt x y`), so `a - b` is correctly PARTIAL — it requires the §28.2 proof `b <= a = True`, which is the deliberate checked-subtraction design (KNOWN_SPEC_ISSUES.md #5), not a missing instance. Unguarded `a - b` over abstract Nats fails `E_UNSOLVED_IMPLICIT` as intended; a guard / saturating helper discharges it. | DONE in `src/Kappa/Prelude.hs`; partiality is spec-correct. |
...
| G28 | 3.1.11, 3.1.10 | RESOLVED (was MINOR) | `suppressed :: [Suppressed]` field; parse-error→cascade suppression and implicit-goal collapse record summaries on the surviving root | DONE: `Diagnostic.hs` + `Pipeline.hs` attachSuppressed + `Check.hs` emitUnsolvedGoal; reachability boundary in KNOWN_SPEC_ISSUES §14 |
...
| G31 | 3.1.14A | RESOLVED (was MINOR) | typed `RecoveryNode` (site/expected/confidence/skipped) now built at each parser recovery site and serialized into the diagnostic's structured `payload` (`kind:"recovery-node"`); cross-decl recovery continues and soundness (no false accept) is preserved | DONE: `src/Kappa/Parser.hs` (`RecoveryNode`, `recoveryDiag`); mirror `tests/conformance/diagnostics/recovery-node-payload.kp` |
| G32 | App. T.8 (T.5.1/T.5.3/T.4) | RESOLVED | structured diagnostic asserts done previously; `assertDiagnosticFixCompiles` now implemented over §3.1.6 fix records (apply+recompile); `runArgs`/`stdinFile` honored (no argv/stdin surface ⇒ behavior = §T.4 defaults; validated, unreadable stdinFile ⇒ harnessError); un-gated `assertStageDump` ⇒ harnessError (no servable checkpoint), gated ⇒ unsupported. No standard directive left at a non-§T.8 `unsupported` | DONE: `src/Kappa/TestHarness.hs` |
| G33 | App. T.5.1 | RESOLVED | numeric or unregistered `<code>`/`<code-or-family>` ⇒ HARNESS-ERROR across all code-bearing directives + `--!!` markers (`assertDiagnosticExplainExists` only rejects numeric) | DONE: `src/Kappa/TestHarness.hs` (`invalidCodeReason`, `requireCode`); covered by `test/Spec.hs` harness mirrors |
| G34 | App. T.6 | RESOLVED | duplicate config key with conflicting value ⇒ ill-formed (HARNESS-ERROR) for `mode`/`moduleMode`/`backend`/`entry`/`runArgs`/`stdinFile`/`dumpFormat`; same value ⇒ fine | DONE: `src/Kappa/TestHarness.hs` (`SConfigKey`, `configConflict`); `test/Spec.hs` mirrors |
| G35 | App. T.2 | RESOLVED (clarified) | a directory that IS a suite root by the suite-root rule (declares `suite.ktest`/`main.kp`, or is named directly) compiles its `.kp` files as ONE unit (verified cross-module); a bare nested dir reached during the recursive walk holds independent single-file inline tests (the conformance tree relies on this, §T.2 form 1) — documented as the spec-faithful reading | DONE: `src/Kappa/TestHarness.hs` (docs); `test/Spec.hs` mirror over a container dir |
| G36 | 4.7 | RESOLVED (was MINOR) | unsafe/debug audit ledger + `assertAuditLedger` query verified: `unsafe_debug/assert-terminates-enabled.kp` and `unsafe-assert-proof-enabled.kp` record the used facility and the harness asserts the ledger contents | DONE: ledger in `Check.hs` (`UnsafeConfig`); harness `assertAuditLedger` in `TestHarness.hs` |
...
## Hostile re-audit verdict (2026-06-15, independent re-probe of all G1..G36)

Each gap re-probed against the built binary
(`dist-newstyle/.../x/kappa/build/kappa/kappa`, mtime unchanged through the
re-audit; no concurrent build). Build re-confirmed `-Werror` clean; in-tree
`kappa test tests/conformance` = **229/229**, `examples` = **1/1**,
...
- **RESOLVED (32):** G1, G2, G3, G5, G6, G7, G8, G9, G10, G11, G12, G13, G14,
  ...
  G32, G33, G34, G35, G36 — all re-probed; original failing probe no longer
...
- **RESOLVED-WITH-RESIDUAL (2) — original probe fixed, a narrower cited MUST
  ...
  - **G18** — `~=`/`Equiv`/fixity present (original `E_NAME_UNRESOLVED` gone),
    ...
    missing)
  - **G20** — 8/10 proof helpers resolve; `measureRelation`/`lexRelation`
