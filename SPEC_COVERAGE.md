...
| §20 Collections, ranges, comprehensions | Partial | Comprehension pipeline lowered over lists per the §20.10.11 as-if rule: `for`/`for?`/`let`/`let?`/`if`, stable `order by` (asc/desc, multi-key), `skip`/`take` with §20.3.2 orderedness tracking (`E_QUERY_UNORDERED_PAGING`), `distinct`/`distinct by`, `group by { aggs [using W] } into`, `join`, `left join … into`; map comprehensions with `on conflict` policies; set/map literals; carrier prefixes `Query`/`OnceQuery`/`SingletonQuery`/`QueryCore mode q`/`Array` with §20.10.1 mode/cardinality/item-quantity plan checks and §20.10.4 row-quantity diagnostics (`queries/clauses_runtime.kp`, `queries/carriers_qtt.kp`). Ranges `..`/`..<`: fixity exists, no prelude implementation. Custom sinks (`FromComprehensionRaw`/`Plan`) require macros: not implemented. |
