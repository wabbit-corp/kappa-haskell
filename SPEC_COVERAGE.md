...
suite, 60/60 passing) or `examples/`. The external black-box corpus
...
| §3.1 Diagnostic records, codes, families | Partial | Structured records with severity, stage, code, `kappa.*` family, primary span, labels, notes, helps, and related origins with the standardized §3.1.1A roles (`Kappa.Diagnostic`). Stable symbolic codes (§3.1.2). Missing: machine-readable registry CLI (§3.1.2A), JSON payloads (§3.1.9), fix-its (§3.1.6 — notes/help only), repair ranking (§3.1.7), provenance frames (§3.1.5A), explanations (§3.1.13). |
| §3.1.14/§3.1.14A Recovery | Partial | Parse-level recovery to declaration boundaries: `parser/recovery.kp` (two errors reported, checking continues). The lexer stops at the first lexical error by design (documented delta; multi-error lexical fixtures in the external corpus under-count). |
...
| §5.5.1.1 Operator sections | Implemented | Left/right/bare sections; `(infix op)`/`(prefix op)` forms parse. Sections usable in check mode (commit `bc08ecd`). |
...
| §6.1 Numeric literals | Implemented | Radix forms (`literals/radix.kp`), fraction/exponent vs suffix split (`literals/float-exponent.kp`, `literals/suffix-term.kp`), §6.1.5 typing/defaulting through `FromInteger`/`FromFloat` (`literals/defaulting.kp`). §6.1.6 Unicode suffixes: gate-off error. §6.1.3: `eqDouble` is raw-bit equality; `floatEq` is IEEE equality. |
...
| §10 ADTs and type aliases | Implemented | Parameterized `data` with named constructor fields, constructor-arity checking (`application/ctor-arity.kp`), named constructor application with field defaults (§10.1.1), transparent aliases (`Int`, `UIO`, …). GADT-style indexed results: "GADT-lite" — the constructor result is unified with the scrutinee type at match (§17.1.2 approximation). |
...
| §11.3 Implicit binders, universalization | Partial | §11.3.3 approximated: free ASCII-lowercase heads in signatures that resolve to no global are implicitly universalized. |
...
| §14 Traits | Partial | User traits, instances, premise instances (`instance Eq a => Eq (List a)`: `traits/premise-instance.kp`), default members (`traits/default-member.kp`). Resolution: local implicit context, then global instance search with a unique-candidate coherence rule (§14.3.1) — ambiguity → `E_INSTANCE_INCOHERENT` (`traits/incoherent.kp`), no candidate → `E_IMPLICIT_UNSOLVED` (`traits/missing-instance.kp`). Not: supertraits/conformance paths (§14.3.3), associated static members (§14.3.4), named instances, instance search-depth control (§14.3.5 beyond a recursion bound). |
| §15 Totality, termination, unfolding | Partial | §15.1 split between total-certified and conversion-reducible is modeled: only reducible definitions δ-unfold during conversion. Structural-descent verification for direct recursion (§15.3 minimum): `recursion/structural.kp` (clean), `recursion/non-structural.kp` (`W_TERMINATION_UNVERIFIED`), recursion without a preceding signature → `E_RECURSION_NO_SIGNATURE` (`recursion/no-signature.kp`). `decreases` measures parse but are not checked; §15.10–15.17 lanes not implemented. |
...
| §16.2 Lambdas | Implemented | Typed/untyped binders, implicit binders, suspension sugar. |
...
| §16.4 Conditionals, flow typing | Partial | `if`/`elif`/`else` expressions (else required in value position: `run/if-expr-needs-else.kp`); statement `if` without `else` in do-blocks (`run/if-no-else.kp`). **Flow-sensitive refinement (§16.4.1–16.4.4) is not implemented** (a large bucket of external-corpus failures). |
...
| §18.2–18.8 do-blocks, kernel | Partial | The §18.8 completion kernel is implemented structurally (`CDo` in core, executed by `Kappa.Interp`): completion records, LIFO `defer` (`run/defer-lifo.kp`), `return` (`run/return-early.kp`), `break`/`continue`, loop `else` only on no-break (`run/loop-else-break.kp`, `run/for-break-continue.kp`), `while` (`run/while-var.kp`), `for`, `let pat <- e` monadic bind, `let? … else` (`run/letq-else.kp`), `var` cells as `Ref` with auto-deref reads (§18.6.1), `!e` splices (§18.3). The only carrier is `IO e` — do-blocks are not generic over user monads. `using` resource binds → `E_UNSUPPORTED`. |
...
| §20 Collections, ranges, comprehensions | Partial | List literals and list comprehensions (`[for x in xs yield e]`, with `if`/`let` clauses). Empty map literal only; non-empty map/set literals, set/map comprehensions, group/order/join query clauses → `E_UNSUPPORTED`. Ranges `..`/`..<`: fixity exists, no prelude implementation. |
...
| §28.2 Normative minimum contents | Partial | Provided: `Void`/`Unit`/`Bool`/`Ordering`/`Option`/`Result`/`List`, aliases `Int`/`Float`/`Char`/`UIO`, `not`/`(&&)`/`(||)` over `Thunk`, `Show`/`Eq`/`Ord` + comparison operators, numeric traits per §28.2.1–28.2.3 (`Add`, `Mul`, `Negatable`, `CheckedSub`/`Div`/`Mod` with proof-carrying `(-)`/`(/)`/`(%)`, `FromInteger`, `FromFloat`), `(++)`, `show`/`print`/`println`/`printString`/`printlnString`, `pure`/`raise`/`throwError`/`catchError`, `identity`, `(\|>)`/`(<\|)`, list functions (`map`, `filter`, `foldl`, `foldr`, `listAppend`, `concatMap`, `listLength`), `orElse`, `newRef`/`readRef`/`writeRef`, `floatEq`. Missing: monad operators (`>>=`, `>>`), `traverse`/`sequence`, equality combinators (`sym`, `trans`, `cong`, `subst`, `pathInd`, `absurd`), `witness`/`summon`, fibers/STM/timers, arrays/sets/maps, ranges, `re`/`b`/`type` handlers, `bracket`. |
