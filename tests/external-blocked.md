...
| `app_reject_explicit_implicit_wrong_type` | `readEnv @1 7`: the literal `@`-payload `1` is supplied to the implicit `Env` binder, and `Env` has no `FromInteger` witness (§6.1.5), so this is correctly rejected with `E_NUMERIC_LITERAL_DOMAIN_MISMATCH` (family `kappa.type.literal-domain-mismatch`, §3.2.3 — the §3.1.4-mandated alias for "no suitable literal witness is available"). The corpus expects an application-argument code; the §3.2 family closest to a mistyped explicit-implicit payload mandates portable `E_EXPLICIT_IMPLICIT_CLASSIFIER_MISMATCH`, which neither implementation emits yet. Either way the rejection is correct and the count matches; only the spelling differs (alignment to the classifier code queued). | §3.2, §3.2.3, §6.1.5, §3.1.4 |
