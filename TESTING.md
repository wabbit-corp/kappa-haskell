...
Current tally over **1115 fixture suites** (one result per fixture;
fresh run 2026-06-20, regenerated by `tools/run-external-fixtures.sh`
into `tests/external-results.md`):
...
These numbers are the authoritative per-run output; `tests/external-results.md`
(regenerated alongside this tally) carries the per-category table, the
§T.8 classification rationale with spec citations, and a "Blocked
classifications" section (maintained in `tests/external-blocked.md`;
`tools/run-external-fixtures.sh --regen` rebuilds the report from the
existing raw log without re-running the corpus).

**The 66 harness errors are real and are NOT a clean bill of health.**
Most are in the `build` category: a fixture asserts another
implementation's §36 build-resolution diagnostic spellings
(`E_BUILD_DEP_UNRESOLVED`, `E_BUILD_LOCK_MISMATCH`,
`E_BUILD_PROVIDER_COLLISION`, `E_BUILD_DEP_RESOLVER_CONFLICT`, …) that
this implementation either spells differently (it emits
`E_DEPENDENCY_LOCK_MISMATCH` / `E_NATIVE_BINDING_UNPINNED` under
`kappa.package.reproducibility`) or does not yet emit at all (dependency
graph / registry / git-dependency resolution and provider-collision
detection are not implemented). Because the asserted code is outside
this implementation's §3.1.2A diagnostic registry, the directive cannot
...
portable-alias table — see KNOWN_SPEC_ISSUES.md #7 on foreign
...
(KNOWN_SPEC_ISSUES.md #7) plus tracked gaps and a small number of cited
...
include the §T.7 `requires capability incremental` fixtures (Chapter 34
session reuse, not claimed) and the deterministic-unsupported forms
(top-level `effect` declarations, `return@`/`defer@`; the §18 scoped
effect + handler mechanism itself is implemented and tested — see
`tests/conformance/effects/`). See the per-fixture tables at the end of
`tests/external-results.md` for the exact breakdown.
