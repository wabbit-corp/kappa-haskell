...
| `assertDiagnosticCodes c1, c2, …`, `assertEval NAME EXPR` | compatibility extensions for the external corpus (§T.1 allows nonstandard directives). `assertDiagnosticCodes` matches codes through §3.1.4 portable aliases in **both** directions: an asserted spelling that is another implementation's nonportable code matches iff both spellings resolve to the same required portable alias. It additionally tolerates a small, explicitly documented set of implementation-defined cross-implementation spelling pairs that are **not** §3.1.4 aliases (currently only `E_UNSOLVED_IMPLICIT ↔ E_TYPE_MISMATCH`; both spellings are implementation-defined per §3.1, and the diagnostic *count* is still compared exactly so behavior is never masked — see `toleranceAliasTable` in `src/Kappa/Explain.hs` and IMPLEMENTATION_NOTES "Review responses"). `assertEval` compares this harness's canonical value rendering, and additionally accepts the corpus formatter's loose list rendering (cons chains unparenthesized in constructor-argument position, e.g. `Some 1 :: Nil` for `Some (1 :: Nil)`). |
