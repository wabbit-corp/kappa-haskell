...
| `assertDiagnosticCodes c1, c2, …`, `assertEval NAME EXPR` | compatibility extensions for the external corpus (§T.1 allows nonstandard directives). `assertDiagnosticCodes` matches codes through §3.1.4 portable aliases in **both** directions: an asserted spelling that is another implementation's nonportable code matches iff both spellings resolve to the same required portable alias. `assertEval` compares this harness's canonical value rendering, and additionally accepts the corpus formatter's loose list rendering (cons chains unparenthesized in constructor-argument position, e.g. `Some 1 :: Nil` for `Some (1 :: Nil)`). |
