...
timeout 600 cabal build -v0 exe:kappa >/dev/null 2>&1 || { echo "FAIL: cabal build"; exit 1; }
...
# no N/expected check.  Keep this bash-3.2-compatible for macOS CI.
bench_n() {
  case "$1" in
    scalar_intloop) echo 10000000 ;;
    scalar_doubleloop) echo 10000000 ;;
    adtbuild) echo 2000000 ;;
    adtmatch) echo 500000 ;;
    byteloop) echo 1000000 ;;
    *) echo "" ;;
  esac
}
...
bench_expected() {
  case "$1" in
    scalar_intloop) echo 50000005000000 ;;
    adtbuild) echo 2000001000000 ;;
    adtmatch) echo 500000 ;;
    byteloop) echo 1000000 ;;
    *) echo "" ;;
  esac
...
PY
  ...
  python3 - "$s" "$e" <<'PY'
import sys
print(f"{float(sys.argv[2]) - float(sys.argv[1]):.4f}")
PY
}
...
for b in "${BENCHES[@]}"; do
  ...
  stats="$(KAPPA_GC_STATS=1 "$exe" 2>&1)"; src=$?
  if [ "$src" -ne 0 ]; then
    printf "%-18s %12s\n" "$b" "RUN-FAIL"
    printf '%s\n' "$stats" | sed 's/^/    /' | head -4
  ...
  if ! { for _ in $(seq 1 "$REPS"); do elapsed "$exe"; done; } | median > "$WORK/$b.nt"; then
    printf "%-18s %12s\n" "$b" "TIME-FAIL"
    fails=$((fails+1)); continue
  fi
  nt="$(cat "$WORK/$b.nt")"
  n="$(bench_n "$b")"; api="-"; [ -n "$n" ] && [ -n "$tb" ] && api="$(awk "BEGIN{printf \"%.1f\", $tb/$n}")"
  # raw-C baseline + ratio, if present
  rct="-"; ratio="-"
  if [ -f "$RAW/$b.c" ]; then
    if $RAWCC "$RAW/$b.c" -o "$WORK/${b}_c" >"$WORK/$b.cc.log" 2>&1; then
      if ! { for _ in $(seq 1 "$REPS"); do elapsed "$WORK/${b}_c" "${n:-}"; done; } | median > "$WORK/$b.rt"; then
        fails=$((fails+1)); rct="RAW-RUN-FAIL"
      else
        rct="$(cat "$WORK/$b.rt")"
