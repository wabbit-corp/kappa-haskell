...
| H-7 | LOW | native runtime / §26.1.4+§27.6 blocking-work bridge | RESOLVED for the binding path. The native profile now DECLARES a runtime capability set (`Kappa.Backend.Capabilities`, §27.6 MUST), every binding carries a §26.1.4 classification (pinned in identity), and the classification→capability rule is enforced fail-closed (`E_BACKEND_CAPABILITY_UNREALIZED`): `blocking-cancellable` is rejected (no safe-cancellation capability), `blocking` is accepted under advertised `rt-blocking`, realized as direct execution on the single agent (the agent IS the blocking lane → §26.1.4:26304 non-starvation holds vacuously). REMAINING (low): a true offloading blocking-work lane that preserves non-starvation if a program both forks concurrently-runnable fibers AND makes a blocking foreign call — currently moot (native `fork` is not a concurrent scheduler). | If/when the native runtime gains a concurrent fiber scheduler, add an offloading blocking lane (or keep failing closed) so a `blocking` call cannot starve co-runnable fibers. |
| H-6 | MED | checker / §9.2 cross-fragment signature satisfaction | §9.2 (Spec.md:7691-7694): a plain top-level signature `name : T` is satisfied **only** by a definition in the **same source file**; a definition in another fragment of the same effective module must NOT satisfy it (`expect` is the cross-fragment form). Actual: a plain signature in one fragment is silently satisfied by a definition in a *different* selected fragment, so the build succeeds where §9.2 requires `E_SIGNATURE_UNSATISFIED`. Surfaced (not exploited) by `examples/native/http_uv` — its facade now uses `expect term` only, which is correct, so the bug is latent. | `csSigPending`/`sigSatisfactionPass` (`src/Kappa/Check.hs:9559-9566,9619-9622`) clear pending signatures against the whole compilation unit's definitions; track the originating source file per definition and discharge a plain signature only with a same-file definition. Risk: touches signature satisfaction used across the conformance suite — gate behind a focused fixture first. |
...
| N-25 | — | DEFER (spec-grounded) | adapter mode `native.direct` IS recorded in the host-binding identity (§26.1.3). A SELECTABLE adapter field is meaningless until a non-native profile exists — the other §26.1.3 modes are jvm.ffm/jvm.jna/dotnet.* which require the jvm/dotnet backend profiles this implementation does not provide (rejected §34.5.3, see Plan.backendUnrealized). Trusted binding summaries (§26.1.5) are a MAY (optional refinement overlay); the conservative defaults this backend uses (Option RawPtr nullability, bare OpaqueHandle only on the raw surface) are spec-compliant WITHOUT a summary | §26.1.3 (per-profile), §26.1.5 (MAY) | revisit when jvm/dotnet profiles land |
...
| N-30 | MED | DONE | the host-binding entry's identity now includes a schema version (`schema host-source-v1`), so a schema change repins (an entry is never reused under an incompatible schema, §36.7:39637); cross-KIND reuse is already rejected (`lockKinds` parse + full-set verify). Dep entries are versioned by the lock header (`v1`). Separator-escaping is a pre-existing non-exploitable nit (the variable-length key is last; ids/kinds are space-free) | §36.7:39604,:39637 | done (escaping nit tracked) |
...
| N-10 | — | DEFER (spec-grounded) | §36.14 C-ABI EXPORT surfaces are a MAY ("an artifact target MAY declare zero or more export surfaces") — not required for an executable target. §36.21 calling convention + data layout are "when applicable": for the C ABI they are DETERMINED by the pinned target triple + toolchain (both in the host-binding identity + threaded to `zig cc -target`), so a single triple does not admit multiple conventions here. ABI language mode (`-std=c11`) is fixed | §36.14 (MAY), §36.21 (when-applicable; triple-determined) | add export surfaces / structured platform roles when multi-profile / C-ABI-export targets are needed |
