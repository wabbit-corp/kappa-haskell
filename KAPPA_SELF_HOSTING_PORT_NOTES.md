...
| H-7 | MED | native runtime / §26.1.4+§27.6 blocking-work bridge | §26.1.4: a `blocking`-classified foreign call "is executed through the backend's blocking-work bridge … unless … already routed through an equivalent blocking lane" and "MUST NOT … starve unrelated runnable fibers". The native runtime executes blocking foreign calls DIRECTLY. Compliant when no other fiber is concurrently runnable (the classification metadata IS carried + pinned, satisfying the §26.1.4 metadata MUST; and direct execution starves nothing in a single-fiber program). NOT implemented: a real blocking-work bridge / fiber scheduler that preserves non-starvation when a program both forks fibers AND makes a blocking foreign call. | Implement a native blocking-work lane (offload a `blocking`-classified call so cooperatively-scheduled fibers keep running), or fail-closed (§27.6 "MUST reject the affected program") when fork + blocking foreign call co-occur and `rt-blocking` is not realized. |
