...
| N-33 | HIGH | open | link/load spec + resolved pkg-config `--libs` are NOT in the host-binding identity (two builds differing only in `link` produce the same pin) | §27.1.1:27368, §36.21:41848,:41861 | thread nbLink/nbLoad + piLibs into the identity |
| N-34 | MED | open | C `double` surfaces as `Double`, not `std.ffi.F64` (asymmetric with F32; double is binary64 = F64) | §26.1.2:26114, §26.2:27297 | add `CtF64`→F64; keep ctDouble as documented ergonomic alias |
