...
| N-20 | HIGH | DONE | `CType` widened to the full §26.1.1 exact-width/unsigned/word-width/float set: ctI8/16/32/64, ctU8/16/32/64, ctIsize, ctUsize, ctF32 (+ existing int/int64/bool/double/string/handle/rawptr). Each maps to its exact C ABI spelling (int8_t…uint64_t/intptr_t/size_t/float) in `cAbiType`; marshalling narrows on unbox / widens on box; integer class surfaces as Integer, floats as Double (§26.1.4). Native suite: ctU32→`uint32_t htonl(uint32_t)` verified against arpa/inet.h | §26.1.1 | done (CChar..CBool nominal std.ffi.c spellings still alias the width classes — tracked) |
...
| N-24 | HIGH | open (subsystem) | RawPtr surfaces as bare `RawPtr` (should be `Option RawPtr` when non-null unproven, §26.1.1); an OWNING handle (e.g. the sqlite db) surfaces as bare `OpaqueHandle` (§26.1.1 forbids for owning). Correct fix needs the §26.1.5 overlay/trusted-summary mechanism (ctor construction in marshalling + ownership annotation) — the §26.1.2/.4/.5 REFINEMENT SUBSYSTEM, which F# also builds incrementally (lane 2b/2c/2g). Conservative typing IS partially honored (UIO results, exact widths) | §26.1.1:27303-27314, §26.1.4 | implement the trusted-summary/overlay subsystem (next major increment) |
| N-25 | HIGH | open (subsystem) | adapter modes (§26.1.3) + trusted summaries (§26.1.5) unmodeled; adapter mode (fixed `native.direct`) absent from the host-binding identity | §26.1.1:26221, §26.1.3, §26.1.5 | add adapterMode field (fold into identity) + trusted-summary input kind; pairs with N-24 |
...
| N-18 | LOW | DEFER | reproducibility verify-only is opt-in (`--locked`), not the package-mode default. §36.7 defines lockfile UPDATE MODES; the default mode is implementation policy (the spec does not mandate locked-by-default). Native ABI is verified fail-closed on EVERY build (discovery in the resolve phase) regardless, and drift is caught under `--locked`. F# chose locked-default; we chose update-on-build — a permitted policy difference, not a MUST | §36.7 (update modes) | flip default if a stricter posture is required |
| N-19 | HIGH | PARTIAL | the binding's REAL dependencies (`verify` decls) are checked against headers fail-closed, and the full symbol surface is digested into the pinned identity (a change forces repin). NOT done: auto-cross-checking every `symbolList` cSymbol against a header (only meaningful for direct-bound, non-shim symbols; shim symbols are package-authored and legitimately absent from system headers) | §27.1.1, §26.1.3 | enforce that a non-shim cSymbol appears in `verify`/headers |
